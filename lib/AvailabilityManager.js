...
class AvailabilityManager {
   ...
   * @param {import('homey-zigbeedriver').ZigBeeDevice} device
   * @param {number} gapMs        - Gap since last seen in milliseconds
   * @param {string} [deviceCardId] - Optional driver-specific DeviceTriggerCard id
   *                                  (e.g. 'ZBMINIR2:device_rejoined').
   *                                  When provided, fires both the global card and the driver card.
   */
  static triggerRejoin(device, gapMs = 0, deviceCardId = null) {
    device.log(`[AvailabilityManager] Firing flow: device_rejoined (gap ${Math.round(gapMs / 1000)}s)`);
    if (!deviceCardId) return;
    ...
    try {
      const card = device.homey.flow.getDeviceTriggerCard(deviceCardId);
      for (const dev of siblings) {
        card
          .trigger(dev, { device_name: dev.getName() })
          .catch(err => device.error(`[AvailabilityManager] ${deviceCardId} trigger failed:`, err.message));
      }
    } catch (err) {
      device.error(`[AvailabilityManager] ${deviceCardId} error:`, err.message);
    }
  }
}
...
class AvailabilityManagerCluster0 extends AvailabilityManagerBase {
  ...
  async install() {
    ...
    try {
      ...
      // Seed last_seen_ts only if absent (new device or store cleared).
      // Keeping the stored value lets the watchdog detect devices that were
      // offline before the app restarted, instead of treating every restart
      // as a fresh start and hiding stale-data for up to timeout minutes.
      if (!this.device.getStoreValue('last_seen_ts')) {
        await this.device.setStoreValue('last_seen_ts', Date.now()).catch(() => {});
      }
