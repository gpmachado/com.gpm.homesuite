...
class NovaDigitalSwitchBase extends ZigBeeDevice {
  ...
  _attachPowerOnGlobalListener(onOffCluster, behaviorKey, currentKey) {
    onOffCluster.on('attr.powerOnStateGlobal', value => {
      ...
      // [Backlight Persistence] Re-enforce OFF after power restore.
  ...
  /** Re-enforce backlight OFF on every boot if user preference is OFF.
   *  Deferred 30 s to let the Zigbee mesh stabilise after app restart. */
  _bootPersistBacklight(onOffCluster) {
    if (!this.getSetting('backlight_enabled')) {
      this.log('[EP1] Init: will re-enforce backlight OFF in 30 s');
      setTimeout(() => {
        if (!this.zclNode) return;
        onOffCluster.setBacklight(false)
          .catch(err => this.log('Re-enforcing backlight off failed (deferred):', err.message));
      }, 30 * 1000);
    }
  }
