...
class SonoffBase extends ZigBeeDevice {
  ...
  /**
   * Start a moderate active poll (Basic cluster zclVersion read) to supplement
   * passive availability tracking for devices that rarely report on their own
   * (confirmed via sniffer: idle ZBMINI-R2 self-reports only a few times per
   * hour). Every successful read feeds AvailabilityManagerPassive through
   * notifyActivity(), on top of whatever the handleFrame hook already sees.
   * Opt-in — call from a subclass's onNodeInit, not run by default here.
   * @param {number} [intervalMs] - Poll interval in ms. Default 5 min.
   */
  _startActivePoll(intervalMs = 5 * 60 * 1000) {
    if (this._activePollInterval) return; // already running (re-init guard)
