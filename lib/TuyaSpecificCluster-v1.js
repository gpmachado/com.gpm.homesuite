...
const COMMANDS = {
    ...
    /**
     * 0x0B: Time response (device → coordinator)
     * Device response to time synchronization command
     * Payload: 2 bytes [0x24, 0x00]
     * Direction: Device → Coordinator
     */
    timeResponse: {
        id: 0x0B,
        args: {
            data: ZCLDataTypes.buffer        // 2-byte payload
        }
    },
...
class TuyaSpecificCluster extends Cluster {
    ...
    /**
     * Handle time response command from device
     * Device sends this as acknowledgment to time sync
     */
    onTimeResponse(response) {
        try {
            this.emit('timeResponse', response);
        } catch (error) {
            this.error('Error handling time response event:', error);
        }
    }
