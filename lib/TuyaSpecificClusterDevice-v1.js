...
const { TimeSilentBoundCluster, BasicSilentBoundCluster } = require('./TimeCluster');
const { getNodeDevices, writeSiblingNames, updateSiblingNames } = require('./connectedDevices');
const TuyaTimeManager = require('./TuyaTimeManager');
...
class TuyaSpecificClusterDevice extends ZigBeeDevice {
  ...
  /** TuyaTimeManager instance for time sync */
  _timeManager = null;
  ...
  async onNodeInit(props) {
    ...
    try {
      ...
      // Suppress "binding_unavailable" log spam from Tuya devices that have
      // an OUTPUT binding to the coordinator's Time cluster (0x000A).
      // endpoint.bind() registers a BoundCluster so the endpoint routes frames
      // there instead of emitting binding_unavailable.
      try {
      ...
      // Initialize TuyaTimeManager for time sync
      this._timeManager = new TuyaTimeManager(this);
   ...
   * Uses TuyaTimeManager for throttling and double shot.
   * 
   * @param {object} [request=null] - Original time request from device
   * @param {object} [options={}] - Sync options
   * @param {boolean} [options.doubleShot=true] - Send twice for LCD MCU compatibility
   * @param {boolean} [options.force=false] - Force sync even if throttled
   */
  async sendTimeResponse(request = null, options = {}) {
    if (!this._timeManager) {
      this.error('[TuyaDevice] TimeManager not initialized');
      return;
    }

    const { doubleShot = true, force = false } = options;

    if (force) {
      this._timeManager.resetLastSync();
    }

    await this._timeManager.sync({
      request,
      doubleShot,
      retries: 2,
      delayMs: 200
  ...
  /**
   * Check if time sync is allowed (throttling check)
   * @param {number} [minIntervalMs=6h] - Minimum interval between syncs
   * @returns {boolean}
   */
  canSyncTime(minIntervalMs) {
    return this._timeManager?.canSync(minIntervalMs) ?? true;
  }
   ...
   */
  getLastTimeSync() {
    return this._timeManager?.getLastSync() ?? 0;
  }
