...
const { TimeSilentBoundCluster } = require('./TimeCluster');
const { getNodeDevices, writeSiblingNames } = require('./connectedDevices');
  ...
  async onNodeInit(props) {
    ...
    try {
      ...
      try {
        ...
        if (ep1?.clusters?.time) {
          ep1.bind('time', new TimeSilentBoundCluster());
        }
      } catch (err) { }
