...
class TuyaZclBase extends ZigBeeDevice {
  ...
  _notifyRejoin() {
    ...
    this.homey.setTimeout(() => this._reEnforceSettings(), 2000);
  ...
  onEndDeviceAnnounce() {
    ...
    // Delay re-enforce to give device time to be ready after rejoin
    this.homey.setTimeout(() => this._reEnforceSettings(), 3000);
  ...
  _reEnforceSettings() {
    ...
    this.log(`[EP1] _reEnforceSettings: backlight_enabled=${backlightEnabled}`);
    ...
    this.log(`[EP1] _reEnforceSettings: indicator_mode=${indicatorMode}`);
    if (indicatorMode && indicatorMode !== 'status' && onOff.setIndicatorMode) {
      ...
      onOff.setIndicatorMode(indicatorMode)
