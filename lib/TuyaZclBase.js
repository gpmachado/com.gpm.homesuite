...
class TuyaZclBase extends ZigBeeDevice {
  ...
  onEndDeviceAnnounce() {
    ...
    if (indicatorMode && indicatorMode !== 'status') {
      // 'status' is the device firmware default — only re-enforce non-default values.
      if (onOff?.setIndicatorMode) {
        this.log(`[EP1] Rejoin: re-enforcing indicatorMode → ${indicatorMode}`);
        onOff.setIndicatorMode(indicatorMode)
          .catch(err => this.log('Re-enforcing indicatorMode failed (rejoin):', err.message));
      }
    }
